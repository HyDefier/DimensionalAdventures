package com.hydefy.dimensionaladventures.common.feature;
...
public class HotIslandFeature extends Feature<BlockStateConfiguration> {
    public HotIslandFeature(Codec<BlockStateConfiguration> p_66017_) {
        super(p_66017_);
    }
        ...
        for(int k = -i; k < i; ++k) {
            for(int l = -i; l < i; ++l) {
                ...
                if (d0 < 0.0D) {
                    ...
                    if (isIcebergState(blockstate) || blockstate.is(Blocks.MAGMA_BLOCK)) {
                        if (p_66040_) {
                            this.setBlock(p_66039_, blockpos, Blocks.LAVA.defaultBlockState());
                        } else {
